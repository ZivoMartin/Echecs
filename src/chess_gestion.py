    ...
    def move_is_possible(self, x1, y1, x2, y2, tab_cases, color, king_coord):
        ...
        for i in range(-2, 3):
            if(i != 0 and (tab_cases[king_coord["x"]+i][king_coord["y"] +(3-abs(i))][0]=="knight" or tab_cases[king_coord["x"]+i][king_coord["y"] -(3-abs(i))][0]=="knight")):
                return False
        i = king_coord["x"] + 1
        while(tab_cases[i][king_coord["y"]][0] == "vide" and i < 8):
            i += 1
            if(tab_cases[i][king_coord["y"]][0] == "rook" or tab_cases[i][king_coord["y"]][0] == "queen"):
                return False
        i = king_coord["x"]-1
        while(tab_cases[i][king_coord["y"]][0] == "vide" and i >= 0):
            i += 1
            if(tab_cases[i][king_coord["y"]][0] == "rook" or tab_cases[i][king_coord["y"]][0] == "queen"):
                return False
        i = king_coord["y"]+1
        while(tab_cases[king_coord["x"]][i][0] == "vide" and i < 8):
            i += 1
            if(tab_cases[i][king_coord["y"]][0] == "rook" or tab_cases[i][king_coord["y"]][0] == "queen"):
                return False
        i = y-1
